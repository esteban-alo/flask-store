...
class GetUsers(ControllerInjector):
    ...
    @user_ns.marshal_with(fields=for_swagger(GetUsersResponseSchema, api=users_ns, operation="dump"),
                          code=200, as_list=False)
    def get(self):
        """ Return all users """
...
class GetUsers(ControllerInjector):
    ...
    @user_ns.response(code=201, description="User data successfully sent", )
    @user_ns.response(code=400, description="Bad Request",
...
class GetUserByName(ControllerInjector):
    ...
    @user_ns.response(code=200, description="", model=for_swagger(schema=UsersSchema, api=user_ns))
    @user_ns.response(code=400, description="Bad Request", model=for_swagger(schema=ErrorSchema, api=user_ns))
    def get(self, name: str):
        """ Get a specific user data by their username """
        try:
            response = self.service.get_user_data(name)
            return marshal(response, fields=for_swagger(schema=UsersSchema, api=user_ns)), 200
        except ValueError:
